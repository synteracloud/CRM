...
class PakistanPaymentAdapter:
    ...
    def __init__(self, merchant_id: str, secret: str, *, stub_mode: bool = True) -> None:
    ...
    def create_payment(self, input: PaymentCreateInput, ctx: AdapterContext) -> PaymentCreateResult:
        ...
        if not self.stub_mode:
            return self._create_payment_live(input, ctx)
    ...
    def _create_payment_live(self, input: PaymentCreateInput, ctx: AdapterContext) -> PaymentCreateResult:
        """Override in concrete subclasses to call real provider API."""
        raise NotImplementedError(
            f"{self.__class__.__name__} must implement _create_payment_live() for production use. "
            "Set stub_mode=True for testing."
        )
