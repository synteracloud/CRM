...
| Token type | TTL | Rotation policy |
|---|---|---|
| Access JWT | 15 minutes | Issued fresh on each refresh grant |
| Refresh token | 7 days | Rotated on every use (previous refresh revoked immediately on use) |
| Service credential token | 1 hour | Re-fetched via client credentials flow on expiry |

Refresh tokens are revoked immediately on: logout, role change, account suspension, admin revocation, or suspicious activity detection.
...
3. Access granted. `break_glass.state_change` audit event emitted (see `docs/observability-audit.md §1.4`).
