<!-- OWNERSHIP
PRIMARY FOR: Idempotency 4-tuple (tenant_id, idempotency_key, request_hash, expires_at) §1.1; event dedup rule (tenant_id, event_name, event_id) §3.1; idempotency ledger schema and query contract.
DEFERS TO: data-architecture.md (transactional outbox — §2.1 step 5 references correctly).
DO NOT RE-DEFINE: Nothing — all other files must point here; this is a leaf-level primary.
-->
...
**Enforcement:** Idempotency records for the following operations MUST (not SHOULD) retain for 7 days: quote acceptance, order creation, invoice posting, payment capture, subscription state transitions. These are enforced by a separate `financial_idempotency_records` partition with a hard minimum TTL of 7 days that ignores the default eviction policy.
...
**Implementation guidance:** Prefer the `409 conflict` with `request_in_progress` response for synchronous APIs (≤5s expected completion). Use block/poll only for async operations where the client is expected to wait. The choice must be consistent within an endpoint — document it in the endpoint contract.
...
**`response_body_ref` format:**

Idempotency records store the canonical response body reference as one of:
- **Inline (small responses ≤ 4KB):** Full JSON-serialized response body stored directly in the `response_body_ref` column.
- **Reference (large responses > 4KB):** A content-addressed storage key (`sha256:{hash}`) pointing to a blob in the platform's object storage. The blob is retained for the idempotency record TTL.

On replay, the system checks `response_body_ref` prefix — if `sha256:`, it fetches from object storage before returning. If the blob has been evicted before the idempotency record expires, the server returns a `409 conflict` with `reason: idempotency_response_unavailable`, signalling the client to treat this as a new request.
...
Use standard error envelope from `docs/infrastructure/api-standards.md`:
...
All events in `docs/infrastructure/event-catalog.md` are subject to inbox dedupe by `event_id` and must preserve producer-generated `event_id` across retries, dead-lettering, and replay.
