<!-- OWNERSHIP
PRIMARY FOR: DSL syntax specification (triggers, conditions, actions, sequencing grammar); DSL parsing and validation rules; workflow_key naming convention (snake_case mapping from workflow-catalog.md names); runtime execution contract for DSL-defined workflows.
DEFERS TO: workflow-catalog.md (canonical workflow names and ordered step semantics — must match); event-catalog.md (canonical event names used in triggers.events); service-map.md (actions.service values).
DO NOT RE-DEFINE: Workflow business logic or step semantics → workflow-catalog.md; canonical event names → event-catalog.md; service ownership → service-map.md.
-->
...
- Stay aligned to the canonical workflow set in `docs/infrastructure/workflow-catalog.md`.
- Support deterministic parsing/validation and runtime execution.

## Alignment with Workflow Catalog

- `workflow_key` MUST match a workflow name from `docs/infrastructure/workflow-catalog.md` using snake_case.
