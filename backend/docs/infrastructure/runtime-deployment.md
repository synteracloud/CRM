<!-- OWNERSHIP
PRIMARY FOR: Health endpoint contracts (/health/live, /health/ready, /health/startup — §3.2 is canonical; observability-audit.md §3.2 points here); environment matrix; runtime environment variable validation rules; deployment rollback strategy; QC gates for deploy readiness; container/infra resource limits.
DEFERS TO: observability-audit.md (log pipeline and metrics hook specs); feature-flags-config.md (flag evaluation SLO values); execution-hardening.md (retry/DLQ behavior in job runners).
DO NOT RE-DEFINE: Structured log event contract → observability-audit.md §1.1; metrics hook list → observability-audit.md §3.1; flag evaluation rules → feature-flags-config.md.
-->
...
- Dual-key overlap window required for zero-downtime rotation.
  **Overlap window duration:** 15 minutes. During overlap, both old and new credentials are accepted. After 15 minutes, old credential is revoked in the secret manager and all instances are expected to have picked up the new credential via secret manager polling (poll interval: 5 minutes max).
...
- Read-only root filesystem (except explicitly mounted writable volumes)
  **Permitted writable volumes:**
  - `/tmp` — ephemeral scratch space (tmpfs, max 100Mi)
  - `/var/log/app` — application log output (if not using stdout)
  - `/data/offline-queue` — offline command queue persistence (sync service only)
  - All other paths: read-only. Writable volume additions require architecture review.
...
**Rollback threshold values:**
- Error rate: rollback if `http_5xx_rate > 2%` sustained for 3 minutes during canary (baseline < 0.5%).
- Latency: rollback if p95 latency increases by > 200ms sustained for 3 minutes during canary.
- Readiness probe: rollback if > 10% of pods fail readiness for > 2 minutes.
- Business KPI: rollback if lead creation rate drops > 30% vs same-hour previous day during canary.
...
- Resource requests/limits set
  **Resource sizing baseline (per service replica):**
  | Service type | CPU request | CPU limit | Memory request | Memory limit |
  |---|---|---|---|---|
  | API/Gateway service | 100m | 500m | 256Mi | 512Mi |
  | Background worker | 50m | 250m | 128Mi | 256Mi |
  | Scheduled job runner | 25m | 100m | 64Mi | 128Mi |
  
  Adjust based on observed p95 CPU and memory from staging load tests. HPA targets 70% CPU utilization. Review and update sizing quarterly.
