<!-- OWNERSHIP
PRIMARY FOR: OCC full specification (version_no increment contract, WHERE clause form, 409 behaviour); pessimistic locking criteria; deadlock prevention rules; entity-level concurrency application map.
DEFERS TO: execution-hardening.md (retry policy after 409); global-idempotency.md (idempotency contract intersecting OCC).
DO NOT RE-DEFINE: Retry backoff values → execution-hardening.md §3.2; transactional outbox steps → data-architecture.md §3.1.
-->
...
- Use **strict aggregate model** (option 1 — bump parent `version_no`) for: `Quote → QuoteLineItem`, `Opportunity → OpportunityLineItem`. Any line item change represents a meaningful commercial change that consumers should observe.
- Use **summary version model** (option 2 — child version + parent summary bump) for: `Case → CaseComment`, `WorkflowDefinition → JourneyStep`. Child changes are internal details; parent summary version reflects externally visible state changes only.
...
- Safe rules:
  ...
  **Privileged override workflow:** To edit amount or close_date on a closed opportunity: (1) Requestor submits override request with reason code via `POST /api/v1/opportunities/{id}/override-requests`. (2) Manager approves via `POST /api/v1/opportunities/{id}/override-requests/{req_id}/approve`. (3) Approved override creates a 15-minute edit window during which the specific fields are unlocked. (4) Immutable audit event `opportunity.override.applied` emitted with before/after values, actor, approver, and reason.
