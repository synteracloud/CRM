<!-- OWNERSHIP
PRIMARY FOR: Canonical workflow names (machine-readable keys used by workflow-dsl.md); ordered step sequences per workflow; service participation maps; trigger event list per workflow; workflow coverage assertions.
DEFERS TO: event-catalog.md (canonical event names and payload schemas — must not redefine); service-map.md (service ownership and boundaries); workflow-dsl.md (machine-readable DSL encoding of these workflows).
DO NOT RE-DEFINE: Event payload schemas → event-catalog.md; service ownership → service-map.md; DSL syntax → workflow-dsl.md.
-->
...
- Workflow sequencing is strictly ordered and references canonical events in `docs/infrastructure/event-catalog.md`.
- No workflow omits intermediate state transitions represented by required events.
- Service participation reflects ownership and consumers from `docs/architecture/service-map.md` and `docs/infrastructure/event-catalog.md`.
...
- **Conversion atomicity contract:** Steps 9-10 (entity materialization) follow a saga pattern. Lead Management Service orchestrates creation in order: Account → Contact → Opportunity (if applicable). If any downstream service fails to materialize its entity within the 10-second timeout:
  1. The `lead.converted.v1` event is NOT emitted.
  2. Any successfully created entities are compensated (soft-deleted) via compensating commands.
  3. The lead status reverts to `qualifying`.
  4. A `lead.conversion.failed` event is emitted with the failure reason.
  5. The operator is notified via a `system_alert` task on the owner dashboard.
  Full re-attempt is manual after operator investigation.
