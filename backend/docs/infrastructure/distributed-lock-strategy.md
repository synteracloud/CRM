<!-- OWNERSHIP
PRIMARY FOR: Redis SET NX implementation detail for distributed locks; fencing token pattern; lock acquisition order (sorted keys) as concrete deadlock prevention implementation.
DEFERS TO: concurrency-control.md (locking criteria — when to use distributed lock vs OCC); execution-hardening.md (retry after lock acquisition failure).
DO NOT RE-DEFINE: Deadlock prevention concept → concurrency-control.md + execution-hardening.md §4.4; 15-min max lease concept → execution-hardening.md §4.3.
-->
...
- **Durability guard:** Every protected operation must also persist an idempotency token in the owning service DB to tolerate lock loss or relay retries.
  **Redis lock + DB token reconciliation:** If a Redis lock is lost (TTL expiry, Redis restart) but the DB idempotency token records the operation as `completed`, the operation is considered successfully completed — no replay. If the DB token is absent or in `processing` state and the Redis lock is gone, the operation is considered interrupted — the scheduler reclaims and retries from checkpoint. The DB token is the authoritative source of truth; Redis lock is the execution guard only.
- **Value payload (JSON):**
...
4. `force_expire(key)` only for operator break-glass with audited reason.
  **`force_expire` authorization:** Requires `platform.ops.write` permission. Caller must provide `{ audit_reason: string, incident_ref: string }`. System verifies no active heartbeat for the key (checks Redis TTL — if key was renewed in the last 5 seconds, `force_expire` is rejected). Audit event `lock.force_expired` emitted with actor, key, reason, incident_ref.
...
3. **Bounded wait:** `wait_timeout_ms` defaults to 2x p95 operation duration (max 5s for synchronous APIs, max 30s for async workers).
  **p95 baseline tracking:** Operation duration baselines are maintained as service-level SLO targets (see `docs/infrastructure/runtime-deployment.md §7.2`). The `http_request_duration_ms` histogram (per `operation_type` label) provides the p95 value. `wait_timeout_ms` defaults are reviewed quarterly and updated in the service configuration map. Hardcoded fallback: 2000ms for synchronous API operations, 15000ms for async workers.
