<!-- OWNERSHIP
PRIMARY FOR: KPI formula definitions (numeric computations for all platform KPIs); aggregation pipeline specs (event-to-metric transformation logic); event-driven KPI update trigger mechanics; time-grain bucketing rules (hour/day/month).
DEFERS TO: domain-model.md (entity definitions — source of truth); event-catalog.md (canonical event names and schemas used in formulas); read-models.md (RM schema for query outputs); employee-performance.md (per-rep KPI layer P-01 through P-08).
DO NOT RE-DEFINE: Entity field definitions → domain-model.md; event payload schemas → event-catalog.md; EmployeePerformanceRM/TerritoryPerformanceRM schemas → read-models.md.
-->
...
- Entities: only from `docs/architecture/domain-model.md`
- Events: only from `docs/infrastructure/event-catalog.md`
...
Pass criteria:
- Every KPI references only event names present in `docs/infrastructure/event-catalog.md`.
- Every referenced payload field exists in that event schema.
- Every business concept maps to entities in `docs/architecture/domain-model.md` (Lead, Opportunity, Quote, Order, Subscription, InvoiceSummary, PaymentEvent).
