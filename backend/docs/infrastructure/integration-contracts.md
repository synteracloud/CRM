<!-- OWNERSHIP
PRIMARY FOR: All approved integration contracts (external APIs, webhooks, data contracts, auth methods); Forecasts API overlay; Plugin Framework overlay; WhatsApp provider contracts; Pakistan payment provider contracts.
DEFERS TO: domain-model.md (base entity shapes in integration payloads); shared-inbox.md (Conversation/thread model); pakistan-adapter-architecture.md (MessagingAdapter and PaymentAdapter interface contracts).
DO NOT RE-DEFINE: Base Conversation entity → whatsapp-execution-model.md; adapter interface contracts → pakistan-adapter-architecture.md.
-->
...
Dedup records are stored in `event_inbox` per `(tenant_id, event_name, event_id)` per `docs/infrastructure/global-idempotency.md §3.2`.
...
All WhatsApp webhooks are normalized by `MessagingAdapter.parseWebhook()` into canonical `MessageWebhookEvent` before any domain processing. See [`pakistan-adapter-architecture.md`](../adapters/pakistan-adapter-architecture.md) for the `MessagingAdapter` interface contract.

---

## Pakistan Payment Provider Contracts

Pakistan payment providers are accessed via the `PaymentAdapter` interface. All payment amounts are in minor units (paisa for PKR). See [`pakistan-adapter-architecture.md`](../adapters/pakistan-adapter-architecture.md) for the `PaymentAdapter` interface contract.
...
**ProviderName values:** `stripe | sendgrid | twilio | meta_whatsapp | twilio_whatsapp | 360dialog | gupshup | jazzcash | easypaisa`
