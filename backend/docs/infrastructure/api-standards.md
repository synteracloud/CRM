<!-- OWNERSHIP
PRIMARY FOR: API shape rules — URL conventions, versioning, pagination, error envelope, HTTP status mappings, field naming, header requirements.
DEFERS TO: global-idempotency.md (idempotency 4-tuple); identity-auth-rbac.md §5.2 (deny-by-default); event-catalog.md (event dedup rule).
DO NOT RE-DEFINE: Idempotency 4-tuple → global-idempotency.md §1.1; deny-by-default policy → identity-auth-rbac.md §5.2; event dedup rule → event-catalog.md §Global Idempotency Processing Rules.
-->
...
- Deny-by-default policy MUST apply. Canonical policy definition: `identity-auth-rbac.md §5.2`.
...
- Canonical idempotency 4-tuple and ledger contract: `global-idempotency.md §1.1`. Do not re-define the 4-tuple here.
...
- Services MUST implement `docs/infrastructure/global-idempotency.md` for API writes and event consumers.
...
- Consumers MUST deduplicate by `(tenant_id, event_name, event_id)` before executing side effects. Canonical dedup rule: `event-catalog.md §Global Idempotency Processing Rules`. Do not re-state the rule here.
