<!-- OWNERSHIP
PRIMARY FOR: Observability pipeline spec (metrics hooks, trace spans, log levels); audit event schema using integrity.hash/prev_hash/chain_seq (canonical — data-governance-layer.md §2.6 must use this schema); GET /api/v1/audits/integrity/verify endpoint.
DEFERS TO: data-governance-layer.md §2.3 (retention durations — primary there); activity-control-model.md (audit hash chain mechanics — consistent with this file); runtime-deployment.md §3.2 (health endpoint contracts — primary there).
DO NOT RE-DEFINE: Retention schedule → data-governance-layer.md §2.3; health endpoint contracts → runtime-deployment.md §3.2 (§3.2 here is a summary pointer only).
-->
...
> **Primary definition:** Health endpoint contracts (`/health/live`, `/health/ready`, `/health/startup`) are owned by `runtime-deployment.md §3.2`. The definitions here are a reference summary only.
