<!-- OWNERSHIP
PRIMARY FOR: Feature flag toggle system (flag registry, evaluation rules, flag-to-tenant config); config management rules; environment-specific override policy.
DEFERS TO: pricing-plans.md (plan→flag mapping table — primary there for which plan enables which flag).
DO NOT RE-DEFINE: Plan tier definitions → pricing-plans.md.
-->
...
### `flag_rule` (Rule schema within `feature_flag.rules[]`)

| Field | Type | Description |
|---|---|---|
| `rule_id` | string | Stable rule identifier within the flag |
| `rule_type` | enum | `tenant_match \| role_match \| percentage_rollout \| env_match` |
| `targets` | array | List of `tenant_id`, `role`, or env values to match (for non-percentage rules) |
| `rollout_percentage` | integer 0–100 | Used only when `rule_type = percentage_rollout` |
| `variant` | enum | `on \| off` — result when rule matches |
| `priority` | integer | Lower number = higher priority; first matching rule wins |

Rules are evaluated in ascending `priority` order. If no rule matches, `default_variant` is returned.
...
### Percentage Rollout Hash Algorithm

```
bucket = SHA-256(flag_key + ":" + subject_id) mod 100
```

- Hash function: SHA-256 (hex digest, first 8 bytes interpreted as uint64, then mod 100).
- Input: `flag_key + ":" + subject_id` where `subject_id` is `tenant_id` for tenant-level rollouts or `user_id` for user-level rollouts.
- Deterministic: same input always produces same bucket. Changing `flag_key` changes distribution.
- Stability: bucket assignment is stable as long as `flag_key` and `subject_id` do not change.
...
- **Change approval**: production flag or config changes require two-person approval.
  - **Change approval process:** Production flag/config changes are submitted as change requests to a dedicated approval queue. Required approvers: two members of `owner_team` (or Platform Engineering on-call for urgency). Approval timeout: 4 hours — escalates to Platform Engineering Lead if not actioned. Emergency override: Platform Engineering Lead can unilaterally approve with mandatory post-incident review within 24 hours.
...
- **TTL policy**: rollout flags must include `expires_at`; expired flags are reported daily.
  - **Expired flag evaluation:** When a flag's `expires_at` has passed and the flag has not been archived, the flag evaluates to `default_variant = off`. A `flag_expired` metric is emitted. The daily expiry report is sent to `owner_team` via the platform notification system. Ops team has 7 days to archive or extend before the flag is auto-archived.
