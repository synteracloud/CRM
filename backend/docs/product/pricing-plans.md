<!-- OWNERSHIP
PRIMARY FOR: Plan tier definitions (Starter/Growth/Business/Enterprise) and PKR pricing benchmarks; feature entitlement matrix (which plan enables which feature flag — this is canonical; feature-flags-config.md defers here for the plan→flag table); plan-gated API enforcement rules (EntitlementGuard); trial period model; metering model and usage limit definitions; TenantUsageMetric context (see also domain-model.md for entity schema).
DEFERS TO: feature-flags-config.md (technical flag toggle mechanism — implementation layer that this doc drives); adoption-ux.md (UX maturity tiers — different concept from billing tiers; do not conflate).
DO NOT RE-DEFINE: Feature flag evaluation algorithm → feature-flags-config.md; UX progressive disclosure tiers → adoption-ux.md; TenantUsageMetric entity field schema → domain-model.md.
-->
