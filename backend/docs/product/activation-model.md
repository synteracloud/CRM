<!-- OWNERSHIP
PRIMARY FOR: "Aha moment < 10 minutes" definition; default 5 activation stages; activation event triggers and KPI thresholds.
DEFERS TO: opportunities-pipeline.md (pipeline stage names); followup-enforcement-model.md (enforcement timing).
DO NOT RE-DEFINE: Opportunity stage names → opportunities-pipeline.md; escalation timing → followup-enforcement-model.md §1.1.
-->
...
1. Tenant provides real WhatsApp provider credentials in Settings → Integrations.
2. System validates credentials via a test API call to the provider.
3. On validation success: real provider webhook is registered; sandbox simulator is deactivated.
4. **During transition (15-minute window):** both sandbox and real webhooks are active. Real inbound messages take priority. Simulated events are suppressed.
5. Any real inbound messages that arrive during the transition window are processed normally — they are NOT treated as sandbox events.
6. After 15 minutes: sandbox mode is fully deactivated. `activation.whatsapp_production_enabled` event emitted.
7. If validation fails: sandbox mode remains active; error displayed with retry option.
...
- Default locale `en`: sample data as defined above (English names + English deal names).
- Default locale `ur` or region `PK` with `ur` locale: sample contact names remain (already Pakistani names); deal names are presented in English with Urdu subtitles pending Urdu string review (see `CONSTRAINTS.md C-010`).
- Sample data is clearly labeled "بیان ڈیٹا / Sample Data" in bilingual environments.
...
**Optimistic status definition:**

| Bootstrap step | Optimistic UI state shown | What's actually pending |
|---|---|---|
| Pipeline creation timeout | Pipeline shown with loading shimmer on stage columns | Server-side stage seeding |
| WhatsApp sandbox provisioning timeout | "WhatsApp ready" shown with pulsing indicator | Sandbox webhook registration |
| Sample data seeding timeout | Placeholder cards shown (grayed out) | Sample contact/deal insertion |

Background retry interval: 3 seconds, max 5 attempts. On final retry failure: error banner shown with "Setup incomplete — click to retry" CTA. User can continue exploring with placeholder UI while retry runs.
