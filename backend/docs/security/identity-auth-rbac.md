<!-- OWNERSHIP
PRIMARY FOR: Full role definitions, permission grants, JWT claims list, token TTL table, auth flow, permission cache TTL (60s), Role-Based UI Config, deny-by-default policy.
DEFERS TO: security-model.md (threat model framing); org-multi-tenancy.md (tenant lifecycle).
DO NOT RE-DEFINE: Tenant isolation invariant → data-architecture.md §1.3; outbox/retry mechanics → execution-hardening.md.
-->
...
- `/docs/infrastructure/api-standards.md`
...
3. Service computes effective roles and permissions from `user_roles` + `role_permissions`.
  - **Conflict rule:** deny overrides allow. If a permission is both granted (via one role) and denied (via policy or missing from another role), the effective result is deny. Computed permissions are cached for 60 seconds with invalidation on `identity.user.role.assigned.v1`.
4. Service issues:
   - short-lived access JWT (`<=15m`)
   - rotating refresh token (persisted in `session_tokens`)
5. Both tokens include `tenant_id` and `jti`.

**IdP token validation rules (step 2):**
1. Verify JWT signature using IdP JWKS endpoint (`JWT_PUBLIC_KEY_URL`).
2. Verify `iss` claim matches configured `JWT_ISSUER`.
3. Verify `aud` claim matches `JWT_AUDIENCE`.
4. Verify `exp` — reject expired tokens.
5. Verify `iat` — reject tokens issued in the future (clock skew tolerance: 30 seconds).
6. Verify `nonce` if present (PKCE flow).
7. Extract `sub` and resolve internal `user` by `(tenant_id, email)`. If user not found in internal DB but exists in IdP: return `403 forbidden` with error code `user_not_provisioned` — triggers provisioning workflow.

**Fallback if IdP unreachable:** Return `503 Service Unavailable` with `Retry-After: 30`. No offline authentication permitted.
...
- `territory_ids` (array of Territory UUIDs the user is assigned to — sourced from Territory & Assignment Service at login; see `territory-management.md §6.2`)
- `iat`
- `exp`
- `jti`
- `aud`
...
- Recompute permissions server-side from authoritative data or verified claims with short TTL.
  - **Permission cache TTL: 60 seconds.** Claims embedded in JWT are trusted for the token lifetime; server-side re-validation of resource scope happens on every request using the `principal_store` backed by Redis (TTL 60s from role-assignment time). On cache miss, load from authoritative DB. If DB unavailable, deny request (fail-closed).
...
*Added from src/role_based_ui overlay — 2026-04-02*

The role-based UI layer implements backend-driven UI visibility contracts. The server resolves which UI sections a principal can see based on their roles and permissions, then returns a typed `UiConfig` payload that the client renders without making its own visibility decisions.

### Type Definitions

| Type | Values |
|---|---|
| `VisibilityMode` | `all` (all required_permissions must match) \| `any` (any permission matches) |
| `AdaptationState` | `expanded \| condensed \| essential` |
| `NavigationMode` | `bottom_nav \| top_bar_with_rail \| left_sidebar` |
| `DensityMode` | `comfortable \| compact` |
| `ElementPriority` | `P0 \| P1 \| P2 \| P3` |
...
| `title` | Display title |
| `route` | App route |
| `required_permissions` | Permissions checked for access |
| `allowed_roles` | Role IDs allowed (empty = all roles passing permissions) |
| `visibility_mode` | `all` or `any` match semantics |

### UiConfig

Resolved UI payload returned to the client after RBAC evaluation.

| Field | Notes |
|---|---|
| `tenant_id` | Tenant scope |
| `principal_id` | Requesting user |
| `role_ids` | Active roles |
| `permissions` | Effective permission set |
| `visible_sections` | Tuple of `UiSectionRule` the principal may see |
| `hidden_section_ids` | Sections explicitly hidden |
| `policy_version` | Version of the RBAC policy evaluated |

### ResponsiveElement / ResponsiveLayout

Responsive layout contracts that adapt the UI to the requesting viewport.

**ResponsiveElement** — `element_id`, `priority (ElementPriority)`

**ResponsiveLayout** — resolved contract for a viewport:

| Field | Notes |
|---|---|
| `viewport_width` | Pixel width of requesting viewport |
| `breakpoint_label` | Named breakpoint (e.g., `mobile`, `tablet`, `desktop`) |
| `adaptation_state` | `expanded \| condensed \| essential` |
| `columns` | Grid column count |
| `navigation_mode` | Navigation pattern for this viewport |
| `density_mode` | Layout density |
| `visible_priorities` | Element priorities rendered (e.g., P0+P1) |
| `collapsible_priorities` | Priorities shown only on expand |
| `sticky_top_bar` | Boolean |
| `sticky_bottom_actions` | Boolean |
| `layout_guards` | Tuple of guard identifiers enforced |
