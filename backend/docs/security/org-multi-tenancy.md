<!-- OWNERSHIP
PRIMARY FOR: Tenant lifecycle (provisioning, suspension, deletion); multi-tenancy admin operations; Admin Control Center spec.
DEFERS TO: identity-auth-rbac.md (auth flow, JWT claims, deny-by-default); data-architecture.md (tenant isolation invariant).
DO NOT RE-DEFINE: JWT claims → identity-auth-rbac.md §3.2; auth flow → identity-auth-rbac.md §3.1; tenant isolation rule → data-architecture.md §1.3; deny-by-default → identity-auth-rbac.md §5.2.
-->
...
*Added from src/admin_control_center overlay — 2026-04-02*

The Admin Control Center is the role-gated administrative surface. Server-side resolution determines which panels a principal can see and whether they have read-only or editable access.

### Type Definitions

| Type | Values |
|---|---|
| `PanelCategory` | `workspace \| users_roles_permissions \| customization \| workflow \| config` |
| `VisibilityState` | `hidden \| read_only \| editable` |

### AdminPanel

Static definition of an admin panel with access requirements.
...
| `name` | Display name |
| `route` | App route |
| `category` | `PanelCategory` grouping |
| `required_permissions` | Read access permissions |
| `write_permissions` | Edit/mutate permissions |
| `critical` | If true, changes require dual confirmation |

### ResolvedPanel

Resolved view of a panel for a specific principal — includes visibility state.

Fields: all `AdminPanel` fields plus `state (VisibilityState)`.

### InteractionPattern

Named interaction pattern with associated controls — used to define standard admin workflows.
...
| `controls` | Tuple of control identifiers |

### AdminControlCenter

Top-level resolved payload for an admin principal.

| Field | Notes |
|---|---|
| `tenant_id` | Tenant scope |
| `principal_id` | Requesting admin user |
| `role_ids` | Active roles |
| `permissions` | Effective permission set |
| `structure` | Ordered panel category identifiers |
| `views` | Tuple of `ResolvedPanel` the principal can access |
| `interaction_patterns` | Available interaction patterns |
| `hidden_panel_ids` | Panels not shown for this principal |
| `policy_version` | RBAC policy version evaluated |
