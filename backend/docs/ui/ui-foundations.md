<!-- OWNERSHIP
PRIMARY FOR: Design tokens (typography scale, spacing scale, color palette, elevation, border-radius); interaction state definitions (view/edit/restricted/audit-only); core UI primitives (buttons, inputs, badges); component state machine rules; role-aware UI behavior anchors.
DEFERS TO: localization.md (RTL layout rules and CSS class conventions — canonical there; must not redefine); capability-matrix.md (surface list this system covers); security-model.md (role-aware interaction constraints); identity-auth-rbac.md (role definitions).
DO NOT RE-DEFINE: RTL/i18n rules → localization.md; role and permission definitions → identity-auth-rbac.md; surface capability list → capability-matrix.md.
-->

# UI Foundations Specification

**Work item:** `B9-P01::DESIGN_SYSTEM_UI_FOUNDATIONS`  
**Inputs:** `docs/architecture/capability-matrix.md`, `docs/security/security-model.md`  
...
## 6) Interaction Simplicity Rule (≤2 Steps)

**Every core CRM action must be completable in ≤2 user steps.**

This is a hard design constraint derived from the system's Pakistan market positioning — users operate in low-attention, high-interruption environments (field sales, mobile-first, low digital literacy).

### Definition of "a step"

A step is any deliberate user action: tap/click, form submission, modal confirmation, or navigation to a new screen. Scrolling within the same screen is not a step.

### Core actions and their ≤2-step targets

| Action | Step 1 | Step 2 | Allowed |
|---|---|---|---|
| Capture a lead from WhatsApp | Message arrives (automatic) | — | 0 steps (auto) |
| Log a follow-up complete | Tap "Done" on task | — | 1 step |
| Snooze a follow-up | Tap "Snooze" → select time | — | 1 step with picker |
| Create a manual lead | Tap "New Lead" → enter phone | Submit | 2 steps |
| Record a cash payment | Open invoice → tap "Record Payment" | Enter amount + submit | 2 steps |
| Advance deal stage | Tap stage chip → select next stage | — | 1 step |
| Send a follow-up message | Tap "Send Follow-up" → confirm template | — | 2 steps |

### Enforcement

- Any CRM workflow requiring > 2 steps for a **core action** (above) must be flagged in design review.
- Complex configurations (custom pipeline stages, team settings, integration setup) are exempt — they are not core actions.
- Forms: inline progressive disclosure preferred over multi-page wizards. Multi-page wizards are only allowed for first-time onboarding flows.

---

## 7) Security-Aligned Design Rules
