<!-- OWNERSHIP
PRIMARY FOR: Role-gated dashboard layout contracts; workspace model definitions (workspace per surface type); dashboard widget catalog; permission-aware component behavior specifications; workspace-level RBAC enforcement rules.
DEFERS TO: ui-foundations.md (design tokens and interaction state primitives — canonical there; must use those tokens); security-model.md (RBAC constraints); capability-matrix.md (surface list this system covers); identity-auth-rbac.md (role and permission definitions).
DO NOT RE-DEFINE: Design tokens or primitives → ui-foundations.md; role/permission definitions → identity-auth-rbac.md; RTL/i18n → localization.md.
-->
...
## Design System Entity Model

*Added from src/design_system overlay — 2026-04-02*

### TokenCategory

`foundation | semantic | domain_alias`

- `foundation` — raw primitive values (colors, spacing, type scale)
- `semantic` — purpose-driven tokens derived from foundation (e.g., `color.action.primary`)
- `domain_alias` — CRM-domain-specific overrides aliased from semantic tokens

### DesignToken

| Field | Notes |
|---|---|
| `token_id` | Unique token identifier (dot-path notation, e.g., `color.action.primary`) |
| `category` | `TokenCategory` |
| `value` | Token value (CSS variable, hex, rem, etc.) |
| `description` | Human-readable description of token purpose |

### ComponentContract

Minimum viable contract for a UI component — defines required and optional interaction states.

| Field | Notes |
|---|---|
| `component_id` | Unique component identifier |
| `primitive_family` | Base primitive (e.g., `button`, `input`, `card`) |
| `required_states` | States the component must handle (e.g., `default`, `hover`, `active`, `disabled`) |
| `optional_states` | States the component may handle if implemented |

### DesignSystemSnapshot

Point-in-time snapshot of the full design system for version tracking.

| Field | Notes |
|---|---|
| `version` | Semantic version string |
| `token_count` | Total token count |
| `component_count` | Total component contract count |
| `tokens` | Tuple of `DesignToken` |
| `components` | Tuple of `ComponentContract` |
| `alias_map` | Dict of `domain_alias_id → semantic_token_id` |
