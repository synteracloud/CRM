<!-- OWNERSHIP
PRIMARY FOR: L1/L2/L3 adapter layer model (authoritative taxonomy); tone tiers polite/firm/urgent; Pakistan provider integration constraints. High-level ComplianceAdapter interface (compliance-adapter.md owns the full ABC).
DEFERS TO: compliance-adapter.md (full ComplianceAdapter ABC); collections-engine-model.md (tone application in collections); whatsapp-execution-model.md (WhatsApp-specific execution).
DO NOT RE-DEFINE: Full ComplianceAdapter ABC → compliance-adapter.md; MessagingAdapter typed interface → whatsapp-execution-model.md §6.2.
-->
...
6. Add integration contracts for new providers to [`integration-contracts.md`](../infrastructure/integration-contracts.md).
...
6. Add the provider's webhook endpoint and contract to [`integration-contracts.md`](../infrastructure/integration-contracts.md).
...
7. Add the provider's webhook contract to [`integration-contracts.md`](../infrastructure/integration-contracts.md).
