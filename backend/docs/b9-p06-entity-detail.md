...
Anchored to `docs/domain-model.md` and canonical events in `docs/event-catalog.md`.
Opportunity Detail and Case/Ticket Detail are also covered in `docs/b9-p03-sales-cockpit.md` and `docs/b9-p04-support-console.md` respectively — this doc provides the canonical standalone contracts.
...
**Primary entity:** `Quote` (contract lifecycle stage) per `docs/cpq-quotes-orders.md`
...
2. **Timeline as activity record:** All canonical events from `docs/event-catalog.md` appear in timeline. Immutable — no delete.
