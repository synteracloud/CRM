<!-- OWNERSHIP
PRIMARY FOR: CQRS-lite pattern §2.1; transactional outbox §3.1; tenant isolation invariant §1.3.
DEFERS TO: global-idempotency.md (idempotency contracts); security-model.md (auth enforcement of isolation).
DO NOT RE-DEFINE: Idempotency 4-tuple → global-idempotency.md §1.1; OCC mechanics → concurrency-control.md; RBAC rules → identity-auth-rbac.md.
-->
...
- Align physical storage with the CRM domain model and service ownership defined in `docs/architecture/domain-model.md`.
...
**Hot tenant overflow strategy:** When a single tenant's write throughput exceeds 20% of a shared shard's capacity (monitored via `db_query_duration_ms` p95 degradation for co-tenants), the following actions trigger:

1. **Alert:** Platform Operations notified via `platform.hot_tenant_detected` alert.
2. **Throttle:** Tenant write rate is throttled to protect co-tenants (returns `429` for non-critical writes).
3. **Rebalance:** Tenant is migrated to a dedicated shard or a less-loaded partition using zero-downtime shard migration (dual-write period of 15 minutes, then read cutover, then write cutover).
4. **Dedicated tier:** High-volume tenants may be provisioned on isolated shards from onboarding if contracted for enterprise tier.
...
**Event schema versioning — consumer handling:**

- `event_version` field on every event (e.g., `"1"`, `"2"`).
- Consumers MUST handle unknown versions gracefully: if `event_version > max_known_version`, log a `warn` and skip processing (do NOT dead-letter — the event is valid, just from a newer producer).
- Breaking schema changes increment the version. The old version is supported for a 90-day deprecation window. After the window, old-version consumers MUST have been upgraded.
- Non-breaking changes (new optional fields) do not increment version. Consumers MUST use defensive field access (treat missing fields as null/default).
...
- Invalidate by domain events:
  ...
  - `identity.user.role.assigned.v1` → evict all permission-context cache keys for the affected `user_id` within their `tenant_id`: pattern `tenant:{tenant_id}:*:user:{user_id}:*`.
  - `feature_flag.updated.v1` → evict all feature flag evaluation cache keys for the affected `tenant_id`: pattern `tenant:{tenant_id}:flags:*`.
