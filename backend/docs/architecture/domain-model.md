...
| TenantUsageMetric | Organization & Tenant Service | Rolling usage counters per feature meter for a tenant — used by EntitlementGuard to evaluate limit checks at runtime. See `pricing-plans.md §6` for context. |
...
### TenantUsageMetric
- **Owner service:** Organization & Tenant Service
- **Fields:** `metric_id (PK)`, `tenant_id (FK->Tenant)`, `meter_code`, `period_start`, `period_end`, `current_value`, `updated_at`
- **Purpose:** Rolling usage counter per feature meter per billing period. Read by `EntitlementGuard` to enforce plan limits at runtime. Written by the Usage Billing pipeline on each metered event.
- **Relationships:** `Tenant` 1-N `TenantUsageMetric`; linked to `TenantEntitlement` via `meter_code`
- **Tenant isolation fields:** `tenant_id`
...
- **Tenant isolation fields:** `tenant_id`
- **Note:** Managed exclusively by the Follow-Up Engine. Manual deletion is blocked; state transitions are system-enforced. See `docs/domain/followup-enforcement-model.md`.
...
Hard-delete is never applied directly — records transition through soft-delete and are subject to retention policy (see `docs/domain/data-governance-layer.md §2.3`).
