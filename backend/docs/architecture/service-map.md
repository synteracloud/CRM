<!-- OWNERSHIP
PRIMARY FOR: Service ownership table — which service owns which API surface, DB, and event namespace; authoritative service dependency graph.
DEFERS TO: domain-model.md (entity names); event-catalog.md (event names).
DO NOT RE-DEFINE: Entity schemas; event payloads.
-->
