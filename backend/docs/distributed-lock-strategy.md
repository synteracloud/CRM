...
3. **Bounded wait:** `wait_timeout_ms` defaults to 2x p95 operation duration (max 5s for synchronous APIs, max 30s for async workers).
  **p95 baseline tracking:** Operation duration baselines are maintained as service-level SLO targets (see `docs/runtime-deployment.md §7.2`). The `http_request_duration_ms` histogram (per `operation_type` label) provides the p95 value. `wait_timeout_ms` defaults are reviewed quarterly and updated in the service configuration map. Hardcoded fallback: 2000ms for synchronous API operations, 15000ms for async workers.
