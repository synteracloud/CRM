...
## UI Config Model (Implementation)

*Added from src/marketing_admin_workflow_ui overlay — 2026-04-02*

### NodeType

`workspace | admin_panel | workflow_ui`

### UiNode

A single navigable node in the integrated marketing/admin/workflow UI graph.
...
| `route` | App route |
| `title` | Display title |
| `node_type` | One of the three NodeType values |

### UiFlowEdge

Directed edge between two UI nodes.

| Field | Notes |
|---|---|
| `source` | Source `node_id` |
| `target` | Target `node_id` |
| `reason` | Navigation reason / trigger description |

### WorkflowUiPane

A pane within the workflow visual builder that surfaces specific controls.

| Field | Notes |
|---|---|
| `pane_id` | Unique pane identifier |
| `title` | Display title |
| `controls` | Tuple of control identifiers available in this pane |

### IntegratedUiModel

Top-level config model for the integrated marketing/admin/workflow UI — composes nodes, edges, and workflow panes into a navigable graph.

| Field | Notes |
|---|---|
| `model_id` | Unique model identifier |
| `start_node_id` | Entry-point node |
| `nodes` | Tuple of `UiNode` |
| `edges` | Tuple of `UiFlowEdge` |
| `workflow_ui_panes` | Tuple of `WorkflowUiPane` |
