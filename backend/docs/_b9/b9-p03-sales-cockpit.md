...
- Pipeline and forecast metrics come from `OpportunityPipelineSnapshotRM` in `docs/ui/read-models.md`.
- Sales execution state and automation cues align to workflow `Opportunity pipeline & close outcomes` in `docs/infrastructure/workflow-catalog.md`.
...
*Added from src/sales_cockpit overlay — 2026-04-02*

### CockpitMetricBinding

Config binding that maps a cockpit metric to a specific read-model field.

| Field | Notes |
|---|---|
| `metric_id` | Unique metric identifier within the cockpit |
| `label` | Display label |
| `read_model` | Source read model class name (e.g., `OpportunityPipelineSnapshotRM`) |
| `field` | Dot-path field name within that read model |

### CockpitView

A single named view within the cockpit with its route and metric bindings.

| Field | Notes |
|---|---|
| `view_id` | Stable identifier (e.g., `pipeline_execution_rail`) |
| `title` | Display title |
| `route` | App route (e.g., `/app/sales/pipeline`) |
| `purpose` | One-line purpose statement |
| `metric_bindings` | Tuple of `CockpitMetricBinding` entries |

### SalesCockpitModel

Top-level workspace descriptor for the sales cockpit — the canonical config object.

| Field | Notes |
|---|---|
| `workspace_id` | `"sales_cockpit"` |
| `title` | Display name |
| `workflow_name` | Anchor workflow: `"Opportunity pipeline & close outcomes"` |
| `primary_jobs` | Tuple of plain-language jobs (progress opportunities, execute close actions, maintain next-action hygiene) |
...
| `views` | Ordered tuple of `CockpitView` entries |

### Canonical Views

| view_id | Route | Read model bound |
|---|---|---|
| `pipeline_execution_rail` | `/app/sales/pipeline` | `OpportunityPipelineSnapshotRM` → `weighted_pipeline`, `stage_distribution` |
| `deal_detail_workspace` | `/app/sales/pipeline/deals/:deal_id` | `OpportunityPipelineSnapshotRM` → `last_activity_at` |
| `forecast_context_rail` | `/app/sales/forecast` | `OpportunityPipelineSnapshotRM` → `forecast_commit_total`, `gap_to_target` |
| `next_actions_panel` | `/app/sales/tasks` | `ActivityTaskOperationalRM` → `overdue_task_count` |
