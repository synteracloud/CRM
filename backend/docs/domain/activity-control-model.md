<!-- OWNERSHIP
PRIMARY FOR: ActivityEvent schema (fields, hash chain, Merkle checkpoint); shadow tracking mechanism; activity log API; immutability guarantees.
DEFERS TO: employee-performance.md (KPI formulas derived from ActivityEvent); observability-audit.md (audit integrity verification layer).
DO NOT RE-DEFINE: Employee performance KPI formulas → employee-performance.md §P-01 to P-08; audit integrity verification API → observability-audit.md.
-->
...
> **Canonical KPI formulas (P-01 through P-08), aggregation rules, `EmployeePerformanceRM` schema, and refresh schedule are defined in `employee-performance.md`.** This section provides a high-level overview only — do not duplicate formula definitions here.

KPI categories derived from ActivityEvent stream:
...
**Shadow tracking** is the guarantee that all business communication captured through the WhatsApp integration is logged in the CRM activity trail — regardless of whether the agent used the CRM interface to initiate or respond to the message.

### Mechanism

The business WhatsApp number's webhook fires for every inbound AND outbound message processed through the provider API. This means:

- An agent replying from their personal device (but through the business number) still generates a webhook event.
- An auto-response sent by the system generates a delivery status event.
- A message sent by another operator on the same conversation is captured.

All such events flow through the MessagingAdapter → ConversationService → ActivityControlEngine pipeline and are logged with actor attribution.

### What Shadow Tracking Does NOT Cover

- Messages sent on a **personal** WhatsApp number not registered as a business number.
- Phone calls, SMS, or emails not routed through the system's adapters.

These out-of-band communications represent an inherent gap in any digital system. The system's response is: **require agents to log contact notes** for any offline interaction via the CRM note-logging feature.

### Anti-Circumvention Implication

Because all business number WhatsApp traffic is captured at the webhook layer, an agent cannot "hide" a conversation with a prospect by simply not opening the CRM. The conversation still flows into the system. This is by design — it closes the lead-leakage gap common in informal sales environments.
