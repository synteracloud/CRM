<!-- OWNERSHIP
PRIMARY FOR: CustomObject definition; schema validation rules; field type registry; custom object API; FieldDefinition (canonical — CustomFieldDefinition merged into this).
DEFERS TO: domain-model.md (base entity extension points); feature-flags-config.md (custom object feature flag gating).
DO NOT RE-DEFINE: Feature flag toggle system → feature-flags-config.md.
-->
...
| `dynamic_fields` | Tuple of `FieldDefinition` — same structure as above; `object_key` is omitted in this context since the object scope is inherited from the parent `CustomObjectDefinition`. `field_type` is an alias for `type` in this embedded form. |

> **Note:** `CustomFieldDefinition` (previously defined as a separate struct) is consolidated into `FieldDefinition`. The two were near-identical; `FieldDefinition` is the single canonical field descriptor for all custom object framework contexts.
