<!-- OWNERSHIP
PRIMARY FOR: Data ownership assignment table — which team/role owns which entity; RACI for data decisions.
DEFERS TO: data-governance-layer.md (retention rules, PII classification — §2 + §3 defer correctly); data-architecture.md (tenant boundary gate).
DO NOT RE-DEFINE: Tenant boundary gate → data-architecture.md §1.2; retention rules → data-governance-layer.md §2.3; PII field classification → compliance-adapter.md PII table.
-->
...
This specification is built from `docs/domain/enterprise-depth.md`, especially the governance lifecycle, non-negotiable invariants, and retention/audit closure points.
...
See `docs/domain/data-governance-layer.md §2.3` — authoritative retention policy table (periods, archive tiers, deletion modes, legal hold behaviour).

---

## 3) Quality Rules

See `docs/domain/data-governance-layer.md §2.4` — authoritative quality dimensions, severity actions, and scoring thresholds.
