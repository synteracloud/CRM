<!-- OWNERSHIP
PRIMARY FOR: KPI definitions P-01 through P-08 (formulas, source events, calculation periods — kpi-data-pipelines.md aggregation engine uses these definitions); EmployeePerformanceRM schema (canonical — read-models.md catalog entry points here); RBAC visibility rules for performance data (who can see which rep's KPIs); KPI refresh frequency for the per-rep layer.
DEFERS TO: activity-control-model.md (raw ActivityEvent source definitions — canonical there); kpi-data-pipelines.md (aggregation pipeline mechanics this layer plugs into); read-models.md (RM catalog — EmployeePerformanceRM registered there).
DO NOT RE-DEFINE: Raw ActivityEvent schema → activity-control-model.md; aggregation pipeline architecture → kpi-data-pipelines.md; RM catalog entries → read-models.md.
-->
