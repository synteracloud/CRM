<!-- OWNERSHIP
PRIMARY FOR: DealRegistration entity and lifecycle; partner tier definitions; commission calculation rules; partner portal API.
DEFERS TO: domain-model.md (base Partner, PartnerRelationship, PartnerAttribution, PartnerCommission fields); event-catalog.md (partner event names).
DO NOT RE-DEFINE: Base partner entity fields → domain-model.md; partner event names and payloads → event-catalog.md.
-->
...
## DealRegistration Entity

*Added from src/partner_channel_management overlay — 2026-04-02*

Records a partner's formal deal registration claim within a time-bounded window.

| Field | Notes |
|---|---|
| `deal_registration_id` | PK |
| `tenant_id` | Tenant scope |
| `partner_id` | FK→Partner |
| `account_id` | FK→Account |
| `opportunity_id` | FK→Opportunity |
| `registered_at` | When the registration was submitted |
| `window_end_at` | Registration protection window expiry |

**Conflict rule:** Overlapping `deal_registration` windows for the same `account_id + partner_id` are rejected. See §Conflict Resolution in the Partner Registration section above.
