<!-- OWNERSHIP
PRIMARY FOR: Canonical payment status enum (full lifecycle); payment processing API; revenue recognition rules; subscription lifecycle; billing event triggers.
DEFERS TO: domain-model.md (base Payment entity fields); collections-engine-model.md (collection-specific payment flows); pricing-plans.md (subscription plan definitions).
DO NOT RE-DEFINE: Base Payment entity fields → domain-model.md; JazzCash/Easypaisa adapter mechanics → pakistan-adapter-architecture.md.
-->
