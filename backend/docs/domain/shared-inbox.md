<!-- OWNERSHIP
PRIMARY FOR: Conversation assignment model (one active agent per conversation invariant); handoff/reassignment protocol (atomic transfer); supervisor visibility and interjection rules; agent presence/availability model; conversation claim semantics (`claim_first` = agent-initiated vs `manual` = supervisor-initiated — these are distinct from cases-domain.md routing labels).
DEFERS TO: whatsapp-execution-model.md (WhatsApp channel mechanics, message delivery — primary there); cases-domain.md (support ticket routing when conversation escalates to case); identity-auth-rbac.md (agent/supervisor role definitions).
DO NOT RE-DEFINE: WhatsApp delivery mechanics → whatsapp-execution-model.md; case routing and SLA → cases-domain.md; opt-out/consent → compliance-adapter.md via whatsapp-execution-model.md §7.4.
-->
...
├── routing_strategy   : InboxRoutingStrategy enum (round_robin | least_loaded | claim_first | skill_based)
  -- Note: `claim_first` (inbox context) = agents proactively claim unassigned conversations from the pool.
  -- `cases-domain.md SupportQueue` uses `manual` for a similar concept (supervisor-driven assignment).
  -- The distinction: claim_first is agent-initiated; manual is supervisor-initiated.
