<!-- OWNERSHIP
PRIMARY FOR: Territory entity schema (full definition — domain-model.md TerritoryRule entry is a pointer here); TerritoryRule criteria types; TerritoryAssignment state machine and immutability rules; lead auto-assignment SLA (1-minute window); default territory configuration rules; territory-scoped query contract (use TerritoryAssignment table, never re-evaluate criteria at query time); territory hierarchy model.
DEFERS TO: domain-model.md (TerritoryRule pointer entity — this doc has full definition); identity-auth-rbac.md §3.2 (territory_ids JWT claim — canonical there); read-models.md (TerritoryPerformanceRM catalog entry).
DO NOT RE-DEFINE: territory_ids JWT claim → identity-auth-rbac.md §3.2; TerritoryPerformanceRM schema → read-models.md; base domain entity catalog → domain-model.md.
-->
