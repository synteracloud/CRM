...
class ActivityControlEngine:
    ...
    def verify_chain_integrity(self, tenant_id: str) -> dict[str, object]:
        """Verify the audit log hash chain is unbroken for a tenant.

        Returns a dict with keys:
          - valid (bool): True if every link's recomputed hash matches stored hash_value
          - entries_checked (int): number of AuditEvent entries examined
          - first_broken_seq (int | None): chain_seq of first broken link, or None
          - errors (list[str]): human-readable descriptions of any broken links
        """
        tenant_log = [e for e in self._audit_log if e.tenant_id == tenant_id]
        tenant_log.sort(key=lambda e: e.chain_seq)

        errors: list[str] = []
        first_broken_seq: int | None = None

        for i, entry in enumerate(tenant_log):
            prev_hash = tenant_log[i - 1].hash_value if i > 0 else "genesis"
            canonical = (
                f"{prev_hash}|{entry.chain_seq}|{entry.tenant_id}"
                f"|{entry.action}|{entry.payload.get('activity', {}).get('result', '')}|"
                f"{entry.payload.get('activity', {}).get('trace_id', '')}|"
                f"{entry.payload.get('activity', {}).get('request_id', '')}"
            )
            expected = f"sha256:{sha256(canonical.encode('utf-8')).hexdigest()}"
            if entry.hash_value != expected:
                errors.append(
                    f"chain_seq={entry.chain_seq} audit_id={entry.audit_id}: "
                    f"stored={entry.hash_value} expected={expected}"
                )
                if first_broken_seq is None:
                    first_broken_seq = entry.chain_seq

        return {
            "valid": len(errors) == 0,
            "entries_checked": len(tenant_log),
