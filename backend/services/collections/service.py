...
from services.core.execution.transactions import TransactionManager, record_payment_event_uow
...
from .stores import EventStore, InMemoryPaymentEventStore, InMemoryPaymentLedgerStore, LedgerStore
...
class CollectionsService:
    def __init__(
        self,
        adapters: dict[str, PaymentAdapter] | None = None,
        ...
        # Injected stores — default to in-memory; swap for DB-backed in production.
        self._event_store: EventStore = event_store or InMemoryPaymentEventStore()
        self._ledger_store: LedgerStore = ledger_store or InMemoryPaymentLedgerStore()
    ...
    def ingest_payment(self, provider: str, signature: str, payload: dict[str, object]) -> tuple[Payment, ReconciliationCase | None]:
        ...
        if self._ledger_store.contains(idempotency_key):
            raise ValueError("Duplicate provider transaction")

        payment = Payment(
            payment_id=f"pay-{len(self._event_store.all())+1}",
        ...
        # Atomic boundary: record payment event + ledger entry together (GAP-018).
        # Stores are injected at construction time — swap InMemory* for DB-backed
        # implementations (see services/collections/stores.py) without touching this method.
        with record_payment_event_uow(
            self._tx,
            event_store=self._event_store,
            ledger_store=self._ledger_store,
            event_data={"payment_id": payment.payment_id, "payment": payment},
            ledger_entry={"ledger_id": f"led-{provider_txn_id}", "key": idempotency_key},
        ) as result:
    ...
    def review_lifecycle_alignment(self) -> ReviewReport:
        checks = {
            ...
            "payments": bool(self._event_store.all()),
