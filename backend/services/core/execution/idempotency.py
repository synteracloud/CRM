...
class GlobalIdempotencyLedger:
    ...
    def evict_expired(self, ttl_seconds: float = 86_400.0) -> int:
        """Remove records whose last_seen_at is older than ttl_seconds.

        Called periodically (e.g. background job / cron) to prevent unbounded
        memory growth.  Only terminal-status records are evicted — in-progress
        records are never evicted regardless of age.
        ...
        with self._lock:
            expired_keys = [
                ...
                if r.status in self.TERMINAL_STATUSES and r.last_seen_at < cutoff
            ]
