...
These tests are skipped in the current suite â€” not failures, but deferred work.
Resolve after CRM is deployed to production and real data/pages are available.

---

## Category 1 â€” Design Gaps (3 tests)
...
## Category 2 â€” Server-Side DataTable Search (3 tests)
...
## Category 3 â€” Selector Mismatch (3 tests)
...
## Category 4 â€” JS Runtime Error (1 test)
A JavaScript error at page load prevents the target component from initialising.

| Test | File | Page | Error | Fix |
|---|---|---|---|---|
| `test_users_table_populated` | `test_func_identity_settings.py` | users.html | `$(...).flatpickr is not a function` â€” flatpickr not loaded in script stack | Add flatpickr JS to users.html script stack (after DataTables, before main.js) |

---

## Category 5 â€” DB_DISABLED Detail Page Tests (7 tests)
...
1. **High** â€” Category 5 (DB detail tests): Auto-resolve on first production DB run. No code changes needed.
2. **Medium** â€” Category 4 (flatpickr): Single line fix in users.html. 30-minute task.
3. **Medium** â€” Category 3 (selector mismatch): Inspect + update 3 test selectors. 1-hour task.
4. **Low** â€” Category 2 (server-side search): Requires backend wiring or DataTable config change.
5. **Low** â€” Category 1 (design gaps): Requires adding filter chip UI to 3 pages.
